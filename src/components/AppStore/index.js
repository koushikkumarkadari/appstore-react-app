...
const appsList = [
  ...
  },
  {
    ...
    imageUrl:
      'https://media.tenor.com/k02ba0xrxiMAAAAM/tom-talking-tom-cat.gif',
    category: 'GAMES',
  },
  {
    appId: 20,
...
class AppStore extends Component {
  state = {searchInput: '', selectedCategory: tabsList[0].tabId}
  ...
  onChangeOfCategoryTab = tabId => {
    this.setState({selectedCategory: tabId})
  }

  render() {
    const {searchInput, selectedCategory} = this.state
    const filteredAppsList = appsList.filter(each => {
      const {category} = each
      return category === selectedCategory
    })
    const filteredAppsListOnSearch = filteredAppsList.filter(each => {
      const loweredAppName = each.appName.toLowerCase()
      const loweredSearchInput = searchInput.toLowerCase()
      return loweredAppName.includes(loweredSearchInput)
    })
    return (
      <div className="background-container">
        ...
        <ul className="tabs-list">
          {tabsList.map(each => (
            <TabItem
              onChangeOfCategoryTab={this.onChangeOfCategoryTab}
              key={each.tabId}
              each={each}
              isActive={selectedCategory === each.tabId}
            />
          ))}
        </ul>
        <ul className="apps-list">
          {filteredAppsListOnSearch.map(each => (
